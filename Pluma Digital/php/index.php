...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_POST['likePost']) && isset($_SESSION['nombre_usuario'])) {
        $usuario_id = getUserProfile($conn, $_SESSION['nombre_usuario'])['id'];
        $publicacion_id = $_POST['publicacion_id'];

        $likeAPI = new LikeAPI();
        if ($likeAPI->likePost($usuario_id, $publicacion_id)) {
            header("Location: index.php");
            exit();
        } else {
            echo "Error al dar like a la publicación.";
        }
    }
}
...
$query = "SELECT p.*, u.nombre_usuario, u.foto_perfil, COUNT(l.id) as likes
          FROM publicaciones p
          LEFT JOIN usuarios u ON p.usuario_id = u.id
          LEFT JOIN likes l ON p.id = l.publicacion_id
          GROUP BY p.id, u.nombre_usuario, u.foto_perfil
          ORDER BY p.fecha_publicacion DESC"; // Agrega esta línea para ordenar los posts
...
<head>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            ...
            const searchBar = document.querySelector('.search-bar'); // Añade esta línea para definir searchBar
            ...
            document.addEventListener('click', function (e) {
                if (!searchBar.contains(e.target)) { // Utiliza la variable searchBar definida
                    searchResults.style.display = 'none';
                }
            });
        });
            ...
            <div class="sidebar-content">
                <div class="profile">
                    <div class="profile-picture"
                        style="background-image: url('<?php echo htmlspecialchars($userProfile['foto_perfil']); ?>');">
                    </div>
        ...
        <section class="main-content">
            ...
            <div class="publicaciones">
                <?php foreach ($posts as $post):
                    $liked = (new LikeAPI())->userLikedPost($userProfile['id'], $post['id']); // Check if the post is liked
                    ?>
                    <div class="tweet-card">
                        ...
                        <div class="tweet-content">
                            ...
                            <?php if (!empty($post['imagen'])): ?>
                                <img src="../uploads/<?php echo htmlspecialchars(basename($post['imagen'])); ?>" alt="Imagen"
                                    class="tweet-image">
                            <?php endif; ?>
                        </div>
                        <div class="tweet-footer">
                            <div class="icon-section" style="display: flex; padding: 0; margin: 0;">
                                <form method="POST" action="">
                                    <input type="hidden" name="publicacion_id" value="<?php echo $post['id']; ?>">
                                    <button type="submit" name="likePost"
                                        style="background: none; border: none; padding: 0; margin: 0;">
                                        <img src="../<?php echo $liked ? 'likedado.png' : 'like.png'; ?>" alt="Like"
                                            style="width: 25px; height: 25px; cursor: pointer;">
                                    </button>
                                </form>
                                <span class="likes-count" id="likes-count-<?php echo $post['id']; ?>"
                                    style="margin-right: auto; margin-top:5px;">
                                    <?php echo htmlspecialchars($post['likes']); ?> Likes
                                </span>

                                <a href="#" class="comment-button" style="padding: 0; margin: 0; margin-left: 15px;">
                                    <img src="../comentarios.png" alt="Comentarios"
                                        style="width: 25px; height: 25px; cursor: pointer;">
                                </a>
                            </div>
                        </div>
